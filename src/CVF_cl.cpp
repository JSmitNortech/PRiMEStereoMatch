...
CVF_cl::CVF_cl(cl_context *context, cl_command_queue *commandQueue, cl_device_id device, Mat *I, const int d) : context(context), commandQueue(commandQueue), maxDis(d)
{
	// OpenCL Setup
	program = 0;
...
int CVF_cl::elementwiseMulDD(cl_mem *cl_in_a, cl_mem *cl_in_b, cl_mem *cl_out)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running MMDD Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::elementwiseMulSD(cl_mem *cl_in_a, cl_mem *cl_in_b, cl_mem *cl_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running MMSD Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::elementwiseDivSD(cl_mem *cl_in_a, cl_mem *cl_in_b, cl_mem *cl_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running MDSD Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::split(cl_mem *cl_I, cl_mem *cl_Ir, cl_mem *cl_Ig, cl_mem *cl_Ib)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running Split Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::sub(cl_mem *cl_in_a, cl_mem *cl_in_b, cl_mem *cl_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running Sub Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::add(cl_mem *cl_in_a, cl_mem *cl_in_b, cl_mem *cl_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running Add Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::central_filter(cl_mem *mean_I_in, cl_mem *mean_cv_io, cl_mem *var_I_in, cl_mem *cov_Ip_in, cl_mem *a_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running central filter Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::preproc_maths(cl_mem *mean_I_in, cl_mem *mean_Ixx_in, cl_mem *var_I_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running variance maths Kernels\n");
	/* Enqueue the kernel */
...
int CVF_cl::boxfilter(cl_mem *cl_in, cl_mem *cl_tmp, cl_mem *cl_out, size_t *globalworksize)
{
	...
	if (OCL_STATS)
		printf("CVF_cl: Running boxfilter row Kernels\n");
	/* Enqueue the kernel */
	...
	if (OCL_STATS)
		printf("CVF_cl: Running boxfilter col Kernels\n");
	/* Enqueue the kernel */
