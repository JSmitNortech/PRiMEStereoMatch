...
void HCI(StereoMatch *sm)
{
	...
	while (key != 'q')
	{
		...
		case 'h':
		{
			...
			printf("|   d:   Dataset: %s\n", sm->MatchingAlgorithm ? "STEREO_GIF" : "STEREO_SGBM");
			printf("|   m:   Computation mode: %s\n", sm->MatchingAlgorithm ? (sm->de_mode ? "OpenCL" : "pthreads") : (sgbm_mode == StereoSGBM::MODE_HH ? "MODE_HH" : sgbm_mode == StereoSGBM::MODE_SGBM ? "MODE_SGBM"
																																																		 : "MODE_SGBM_3WAY"));
		...
		case 'm':
		{
			...
			{
				sgbm_mode = (sgbm_mode == StereoSGBM::MODE_HH ? StereoSGBM::MODE_SGBM : sgbm_mode == StereoSGBM::MODE_SGBM ? StereoSGBM::MODE_SGBM_3WAY
																														   : StereoSGBM::MODE_HH);
				sm->ssgbm->setMode(sgbm_mode);
				printf("| m: STEREO_GIF Matching Algorithm:\n");
				printf("| m: Mode changed to %s |\n", sgbm_mode == StereoSGBM::MODE_HH ? "MODE_HH" : sgbm_mode == StereoSGBM::MODE_SGBM ? "MODE_SGBM"
																																		: "MODE_SGBM_3WAY");
			}
			break;
		}
